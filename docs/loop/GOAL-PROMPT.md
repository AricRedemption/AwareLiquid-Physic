...
- 算力闸门 v3(AMM-008,硬,每轮动手前必过)分四档:**T0 零算力**(判读/文档/文献扫描/秒级探针,默认档,每条 <10min);**T1 微探针**(≤15min/条,每轮算力合计 ≤30min:小样本探针训练/小评测,当轮串行执行,跑完即判读);**T2 本机探针训练**(预计 15–60min:先预注册进 PRD §19[动机/判负标准/seeds/完整命令/预计时长]→ `./scripts/probe_run T2 <est_min> -- <命令>` 护栏后台执行[日志落盘,同一时刻仅 1 个]→ 马拉松继续零算力轮绝不阻塞等待,次心跳机械验收);**T3 云档**(>60min 预计 或 全量/多 seed 终局:预注册后做成 PR 交付云算力,PRD §19 登记欠账)。**T1/T2 结论只解锁路由决策与假设筛选,不得单独支撑 N1 终局声明**——终局声明必须 T3 或隐藏卷一次性复验;结果 JSON meta 必带 exec_tier/tier_est_min/threads/nice。
