...
| `r1d_mode_scan.py` | **R1d 逐模式提取扫描**(轮 122,R1D-MODE-SCAN):SB-NAMING 可证伪预言检验——e1_final 逐项同协议(同池同 seed0,300 步 semigroup,液态算子,mse 与 e1_final seed0 逐位一致)训练后逐 ctx 坐标归一化提取误差(线性解码,field_context_probe 同约定),sin 模式 1..4 排秩 Spearman ρ 三段门(≥0.6 过/≤0 判负⇒命名降级+第四路删除/中间弱趋势待 3-seed);副指标逐模式归因(不作门);判读 NEGATIVE=全模式无可解码信号(corr 噪声级,e≈1,与 e1_final corr 0.014/D-3 0.058/P3 链一致),判负分支机械执行;1-seed 筛查口径 | `probe_run T1 10 -- .venv/bin/python benchmarks/r1d_mode_scan.py`(~3min 实测);测试 `tests/test_r1d_mode_scan.py` |
