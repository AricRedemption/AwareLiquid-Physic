...
| `warmup_probe.py` | **lr warmup 对照探针**(轮 212,WARMUP-PROBE):异频池 2000 步 prefix 两臂——A 恒定 lr=3e-3 vs B warmup(前 200 步线性 0→3e-3,LambdaLR 注入=轮 181 哨兵条款适用);预注册三分支判据(\|diff\|<5%=UNRESOLVABLE 与轮 194 不在 EOS 一致/B 好≥5%=BENEFICIAL/B 差=HARMFUL);实测 WARMUP_BENEFICIAL(A 3.5582 vs B 1.9843,好 44.2%)=**warmup 显著有益,与轮 194 SHARP_BELOW 预测张力显性化(机制候选:早期瞬态 sharpness 或噪声冲击规避,未分离)**;A 臂与历史三重逐位一致=交叉验证锚;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/warmup_probe.py`(~2min 实测);测试 `tests/test_warmup_probe.py` |
