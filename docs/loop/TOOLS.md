...
| `recipe_synthesis_probe.py` | **组合配方回灌探针**(轮 227/228,RECIPE-SYNTHESIS;AMM-028 门 2 首例+门 3 统计升级首例):全默认 vs 单轴最优合成(depth4+lrdecay0.999+wd1e-4+warmup200+ktrain4)两臂×3-seed;train_recipe=房史 prefix 循环三旋钮组合(中性参数下与房史 train 逐位一致,测试钉死=轮 181 哨兵);判读 ratio 三分支 SYNERGIC/NULL/ANTAGONISTIC 预注册,判读行附方向一致性+seed 间 spread;实测 RECIPE_SYNERGIC ratio=0.893 收益 10.7%(3/3 方向一致)=单轴不可加实证 | `probe_run T1 15 -- .venv/bin/python benchmarks/recipe_synthesis_probe.py`(~2min 实测);测试 `tests/test_recipe_synthesis_probe.py`(边界 19/20 与 21/20 精确构造) |
