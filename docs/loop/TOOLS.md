...
| `m1_cap_axis_probe.py` | **M1 容量轴探针**(轮 113,M1-CAP-AXIS,dir/m1-cap-axis):d_model∈{24,48,96}×3 seeds×双训练环(prefix/all2all),判读=liquid edge 随容量走向;**复用 d1b 同一代码路径(run_one)实现桥接逐位复现**;实测 r 2.40→2.31→1.53(差距收窄未消失,险过门限 1.5,弱结构效应判定)+符号反转 1/3 容量恒定(开放问题首条容量轴数据);预注册判负①-③钉在代码内 | `probe_run T1 25 -- .venv/bin/python benchmarks/m1_cap_axis_probe.py`(实跑 13min);测试 `tests/test_m1_cap_axis_probe.py` |
