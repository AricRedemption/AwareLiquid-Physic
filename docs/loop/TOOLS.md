...
| `nscales_ladder_probe.py` | **n_scales 尺度数阶梯对照探针**(轮 197,NSCALES-LADDER):异频池 2000 步 prefix 四臂 n_scales∈{1,2,4,8}(LTC 核构造参数天然可注入=轮 181 哨兵条款适用);预注册 spread 判据(<1.05=UNRESOLVABLE n_scales=4 默认充分/≥1.05=RESOLVED 报告最优);实测 NSCALES_RESOLVED(1=3.13/2=2.44 最优/4=3.56/8=3.81,spread 1.56,内点最优)=**默认 n_scales=4 非最优且参数量混杂排除(+8% 参数非效应载体)=纯架构结构效应**;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/nscales_ladder_probe.py`(~2min 实测);测试 `tests/test_nscales_ladder_probe.py` |
