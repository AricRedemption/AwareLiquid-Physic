...
| `sharp_probe.py` | **sharpness 轨迹探针**(轮 154,SHARP-PROBE):M1 弹簧同池 hidden64 checkpoint 各估固定 128 轨全窗口 loss 的 λ_max(HVP 幂迭代 20 步,双反向 autograd,allow_unused 零填充);预注册四值判据(不收敛/非有限⇒UNRESOLVABLE 判负;λ_max·lr∈[1.5,3]=EOS/<1.5=BELOW/>3=ABOVE,Adam 修正面如实注记);v1 实测 SHARP_BELOW(0.009→0.066);**轮 156 SHARP-PROBE-2 叠支延长轴 {0..10000}**:全程 <0.1(末端 0.082,差 24×)rising 无悬停=训练全程深度稳定区,轮 146 反弹 EOS 解释对全程排除,"稳定但噪声可观"体制定性收口(GNS 末端 B_simple~33<64 互证);1-seed 筛查口径;checkpoint 轴/out_name CLI 参数化 | `probe_run T1 10 -- .venv/bin/python -u benchmarks/sharp_probe.py --checkpoints 0,1000,2000,4000,6000,8000,10000 --out_name sharp_probe_v2.json`(~4min 实测);测试 `tests/test_sharp_probe.py` |
