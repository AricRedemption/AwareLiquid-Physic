...
| `map_vs_flow.py` | **学习对象 dt 迁移探针**(轮 129,MAP-VS-FLOW):M1 裸头 dt=0.1 训练后于 dt∈{0.05,0.1,0.2} 固定视距 T=10(轮 104 口径)评估——学到 H 向量场则跨 dt 一致(1-step 误差 O(dt²)+滚出比 ≤2),学到 dt 映射则 O(1) 崩塌;单频控制族(轮 120 先例)+dt=0.05 原生参照臂;实测 ratio 0.895+斜率 +1.995 ⇒ FLOW_LIKE,N1"学到 H"措辞维持(扫描 dt 倍频内限定);判负=MAP_LIKE ⇒ 措辞降级映射对象;1-seed 筛查口径 | `probe_run T1 5 -- .venv/bin/python benchmarks/map_vs_flow.py`(~1.5min 实测);测试 `tests/test_map_vs_flow.py` |
