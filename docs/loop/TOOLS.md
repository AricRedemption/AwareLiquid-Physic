...
| `wsa_probe.py` | **尾段权重平均对照探针**(轮 209,WSA-PROBE):异频池 2000 步 prefix 注入式训练,尾段 10 检查点(每 100 步)均匀平均 vs 最后 checkpoint,同 held-out k100 评估;预注册三分支判据(发散⇒UNRESOLVABLE/\|diff\|<5%=不可分辨/B 好≥5%=SWA_BENEFICIAL/B 差⇒SWA_HARMFUL);实测 SWA_HARMFUL(AVG 3.118 vs LAST 2.869,差 8.0%)=本仓体制尾段平均有害,与轮 126 符号盆地一致(平均跨盆地=损害);**偏差如实注记=注入式循环与 train_prefix RNG 流差异,LAST 臂 2.869≠历史 3.558,跨轮绝对数字不可比(轮 149 条款边界)**;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/wsa_probe.py`(~2min 实测);测试 `tests/test_wsa_probe.py` |
