...
| `opt_compare_probe.py` | **优化器对照探针**(轮 178,OPT-COMPARE):异频池 2000 步 prefix 两臂——A Adam lr=3e-3(默认)vs B SGD-momentum lr=0.1(常用值),lr 未逐优化器调优=受限对照(预注册局限);四分支判据(发散⇒OPT_UNRESOLVABLE=lr 预算未调平/\|diff\|<5%⇒不可分辨/SGD 好≥5%=自适应代价/Adam 好≥5%=自适应增益);实测 OPT_SGD_BETTER(SGD 好 8.6%,Adam train_loss 更低 2.4× 但不传递)=训练-泛化解耦第三实例(轮 146/159 同族 n=3),Wilson 2017 方向;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/opt_compare_probe.py`(~1min 实测);测试 `tests/test_opt_compare_probe.py` |
