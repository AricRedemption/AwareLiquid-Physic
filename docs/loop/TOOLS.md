...
| `direction_gate` | **方向门禁(第六门)判单校验器**(轮 231,AMM-029 用户提议):执行会话每轮提交前追加判单(JSONL 五字段:round/direction∈{ALIGNED,DRIFT,UNCERTAIN}/evidence 引用本轮产物/effective_output/next_adjust)再 `--check`;缺判单/坏枚举/ALIGNED 无证据/与 NONE 矛盾 ⇒ exit 1 禁提交,**连续 2 条 DRIFT ⇒ exit 3 强制升级 BLOCKED-HUMAN**;判单入 git 供夜账抽样复核;诚实边界=判官为执行会话自评(自我裁判偏差),独立判官会话=停车场 | `scripts/direction_gate --check`(提交门,与 pytest/audit 并列);`--add '<json>'` 追加判单;测试 `tests/test_direction_gate.py`(12 用例含漂移升级) |
