...
| `wd_ladder_probe.py` | **weight decay 阶梯对照探针**(轮 181,WD-LADDER):异频池 2000 步 prefix 三臂 Adam wd∈{0,1e-4,1e-2}(注入式训练循环——train_prefix 内部 optimizer 硬编码 wd=0,首跑三臂逐位一致哨兵暴露=假判读拦截);预注册 spread 判据(<1.05=UNRESOLVABLE §44.3 相容/≥1.05=RESOLVED 报告最优 wd);实测 WD_RESOLVED(spread 1.18,最优 wd=1e-4 内点,默认 wd=0 差 17.6%)=适度正则 rollout 增益;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/wd_ladder_probe.py`(~2min 实测);测试 `tests/test_wd_ladder_probe.py` |
