...
| `residual_spec_probe.py` | **rollout 残差频谱诊断**(轮 219,RESIDUAL-SPEC):默认配置(prefix hidden64 ctx=8 2000 步)held-out 128 轨 k=200 rollout 残差 FFT 功率谱平均;诊断轮无通过/失败门——谱读数报告即交付(主峰频率 vs 真值基频带 0.011-0.029 cyc/step+高频能量占比+谱形);实测 RESIDUAL_PROFILED:hf_ratio=0.932(93% 残差能量在高频段)主峰 0.199 远高于基频带=高频欠拟合主导(§17 谱偏置评估端镜像),标量 MSE 被高频残差主导;gen_steps 须 ≥t_obs+rollout_k+1(轮 162 坑再证);1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/residual_spec_probe.py`(~2min 实测);测试 `tests/test_residual_spec_probe.py` |
