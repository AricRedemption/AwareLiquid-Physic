...
| `grok_curve.py` | **训练量-泛化函数形状探针**(轮 146,GROK-CURVE):M1 弹簧同池 held-out 口径(E1 协议步数轴版,run_one/gen_spring 逐字复用)hidden64 步数阶梯 {2500..40000} 逐点 k100 rollout MSE+train_loss;预注册双条件判据(相邻 ≥3× 跌落 **且** 跌落点 train_loss≤1e-4 饱和)⇒ GROKKING_SIGNATURE/SMOOTH_ASYMPTOTE;实测判负分支执行:全阶梯无 ≥3×(最大 1.42×)且 train_loss 未饱和(grokking 命名不适用本仓,曲线非单调属 1-seed 噪声面) | `probe_run T1 25 -- .venv/bin/python -u benchmarks/grok_curve.py`(~21min 实测,勿低于 est——跨循环类型标度外推曾低估 5×);测试 `tests/test_grok_curve.py` |
