...
| `amp_extrap_probe.py` | **初始幅度外推对照探针**(轮 216,AMP-EXTRAP):训练 scale=1 标准池,评估 scale{1,2,4} 幅度缩放池(初条件能量 ∝scale²)各 held-out k100;**相对口径必需**(绝对 MSE 随能量平方增)rel_mse=rollout/信号能量;预注册 rel_comp 判据(<3=ROBUST/≥3=DEGRADES);实测 AMPEX_DEGRADES(rel_comp=4.08:rel 1.80→4.86→7.32 单调退化)=幅度是插值域边界维度;**深层读数=谐振子线性性下幅度缩放是精确解缩放,模型未继承该不变性=学到的映射有非线性偏差(可检测机制线索)**;scale=1 逐位一致校验=轮 181 哨兵条款;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/amp_extrap_probe.py`(~2min 实测);测试 `tests/test_amp_extrap_probe.py` |
