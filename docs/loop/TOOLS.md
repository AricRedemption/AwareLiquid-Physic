...
| `gns_probe.py` | **梯度噪声尺度闭式估计探针**(轮 149,GNS-PROBE):M1 弹簧同池 hidden64 三个训练进度 checkpoint(0/1000/4000 步 prefix 短训)各采 N=32 个 batch-64 随机子批梯度(loss 与训练循环逐字同构),一阶估计 B_simple=tr(G)/|ĝ|²;预注册三值判据(零梯度/非有限⇒UNRESOLVABLE 判负,跨 checkpoint ≥3×⇒RESOLVED_TREND,否则 FLAT 量级保留);实测 GNS_RESOLVED_TREND 增长(11.9→83.1→60.4,~7×),batch=64 早期噪声主导区、训练后近线性加速临界=轮 126 种子敏感性的噪声侧一致性读数;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/gns_probe.py`(~3min 实测);测试 `tests/test_gns_probe.py` |
