...
| `icl_m3.py` | **少样本适配三臂对照**(轮 132,ICL-M3):M3 wave 族 c=1.5 未见任务——A 前缀摊销零样本(隐式 ICL)/B pretrain+finetune(GrBAL 梯度式)/C 从头 n-shot 对照;M3 真实协议 300/200/400 步(此前唯一产物为 5 步冒烟);判负①增益方向②排序可判机械门;实测 B<C<A(1.195e-2/1.336e-2/5.819e-2)增益方向成立+梯度适配增量为隐式 4.9×(c_target 外插混杂如实注记);1-seed 筛查口径 | `probe_run T1 10 -- .venv/bin/python benchmarks/icl_m3.py`(~2min 实测);测试 `tests/test_icl_m3.py` |
