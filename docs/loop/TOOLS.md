...
| `sharp_probe.py` | **sharpness 轨迹探针**(轮 154,SHARP-PROBE):M1 弹簧同池 hidden64 checkpoint {0..2000} 各估固定 128 轨全窗口 loss 的 λ_max(HVP 幂迭代 20 步,双反向 autograd,allow_unused 零填充);预注册四值判据(不收敛/非有限⇒UNRESOLVABLE 判负;λ_max·lr∈[1.5,3]=EOS/<1.5=BELOW/>3=ABOVE,Adam 修正面如实注记);实测 SHARP_BELOW(λ_max·lr 0.009→0.066 ≪2,差 30×)=本仓训练不在 edge of stability,轮 146 非单调反弹的 EOS 振荡解释排除;趋势 rising(λ_max 7.8× 未及悬停);1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/sharp_probe.py`(~3min 实测);测试 `tests/test_sharp_probe.py` |
