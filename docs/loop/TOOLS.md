...
| `spectral_dt_probe.py` | **谱偏置×离散化交叉双因子矩阵探针**(轮 162,SPECTRAL-DT):单频弹簧池(gen_spring ω_lo=ω_hi,轮 120 控制臂同法)ω{1,2,4}×dt{0.05,0.1,0.2} 九单元 2000 步 prefix,固定物理视距 T=10(k=200/100/50)归一 rollout MSE;预注册交互判据(高频粗/细 dt 增幅比超低频同比 ≥20%);实测 INTERACTION_RESOLVED(R4=24.1 vs R1=0.049,+2410%):恶化集中于唯一近 Nyquist 单元(ω·dt=0.8)=训练网格不足"学不到"非"测不到";ω=1 粗 dt 反而最好(每步物理覆盖效应,轮 129 一致);训练网格解析度条款入 N1;1-seed 筛查口径 | `probe_run T1 12 -- .venv/bin/python -u benchmarks/spectral_dt_probe.py`(~5min 实测;gen_steps 须 ≥t_obs+T/dt_min+1);测试 `tests/test_spectral_dt_probe.py` |
