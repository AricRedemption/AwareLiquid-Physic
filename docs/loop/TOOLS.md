...
| `sign_flip_probe.py` | **半群符号反转异常定位探针**(轮 126,SIGN-FLIP-PROBE):d1b/cap-axis 同池几何(d48 固定)跨 8 seed 扫描 prefix/all2all 逐 seed 比 r=mse_all2all/mse_prefix,翻转=r<1;cap-axis d48 逐 seed 比逐位复现锚+2× 预算稳定臂(上限前 4 翻转 seed,最坏 T1 时长保护)+损失比等价解签名(≤2=盆地/欠拟合混淆门);四值机械判读(BASIN_STABLE/TRANSIENT/MISFIT/NOT_REPRODUCED);实测翻转 3/8(≈记录 1/3)、seed1/3 盆地稳定+seed7 瞬态+损失比全≤0.23⇒机械 TRANSIENT 异质纹理如实;诊断口径,多 seed 终局=停车场 | `probe_run T1 20 -- .venv/bin/python benchmarks/sign_flip_probe.py`(~16min 实测);测试 `tests/test_sign_flip_probe.py` |
