...
| `omega_extrap_probe.py` | **ω 带外外推探针**(轮 115,OMEGA-EXTRAP,dir/omega-extrap):M1 训练于 ω∈[0.7,1.8],带内 anchor+带外低/高带评估 k100 MSE+ctx→ω 闭式线性解码,liquid/static 双臂;实测"结构保持、推断退化"分离(liquid 全带绝对占优 0.77/7.53/2.65 vs static 2.33/9.76/5.90,ctx 解码带外 corr≈0),N1 scope 升级量化退化曲线 [B];预注册判负①-③全部相对判据(轮 74 条款,house 无带外先例不设绝对魔数) | `probe_run T1 10 -- .venv/bin/python benchmarks/omega_extrap_probe.py`(实跑 ~3.5min);测试 `tests/test_omega_extrap_probe.py` |
