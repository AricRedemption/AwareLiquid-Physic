...
| `gns_probe.py` | **梯度噪声尺度闭式估计探针**(轮 149,GNS-PROBE):M1 弹簧同池 hidden64 训练进度 checkpoint 的 N=32 个 batch-64 随机子批梯度(loss 与训练循环逐字同构),一阶估计 B_simple=tr(G)/|ĝ|²;预注册三值判据(零梯度/非有限⇒UNRESOLVABLE 判负,跨 checkpoint ≥3×⇒RESOLVED_TREND,否则 FLAT 量级保留);v1(3 checkpoint)实测 GNS_RESOLVED_TREND 增长(11.9→83.1→60.4),轮 126 种子敏感性噪声侧读数;**轮 151 GNS-PROBE-2 叠支延长轴 {0..20000}**:83(1k 峰值)→60→68→**33.45(20k,回落<64)**=长训练体制 batch=64 重新落入噪声主导侧,T2/T3 派发 batch 字段读数到手;1-seed 筛查口径;checkpoint 轴/out_name 全 CLI 参数化=后续池迭代零新代码 | `probe_run T1 15 -- .venv/bin/python -u benchmarks/gns_probe.py --checkpoints 0,1000,4000,10000,20000 --out_name gns_probe_v2.json`(~6min 实测);测试 `tests/test_gns_probe.py` |
