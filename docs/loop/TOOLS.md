...
| `lrdecay_ladder_probe.py` | **lr 衰减阶梯对照探针**(轮 201,LRDECAY-LADDER):异频池 2000 步 prefix 三臂 lr_decay∈{1.0 恒定,0.999,0.99}(prefix 参数天然可注入=轮 181 哨兵条款适用);预注册 spread 判据(<1.05=UNRESOLVABLE 恒定默认充分/≥1.05=RESOLVED 报告最优衰减率);实测 LRDECAY_RESOLVED(最优 0.999 内点,恒定 1.0 差 17.9%)=**train.py 内部断言"恒定 lr 过拟合长日程"获实证支持**;lr_decay 1.0 臂与轮 175/178/181 逐位一致=三重交叉验证锚;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/lrdecay_ladder_probe.py`(~2min 实测);测试 `tests/test_lrdecay_ladder_probe.py` |
