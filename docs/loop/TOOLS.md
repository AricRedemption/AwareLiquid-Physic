...
| `depth_ladder_probe.py` | **深度阶梯对照探针**(轮 185,DEPTH-LADDER):异频池 2000 步 prefix 三臂 depth∈{1,2,4}(hidden 固定,构造参数天然可注入=轮 181 哨兵条款适用);预注册 spread 判据(<1.05=UNRESOLVABLE depth=2 默认充分/≥1.05=RESOLVED 报告最优 depth);实测 DEPTH_RESOLVED(depth 4.73→3.56→2.44,spread 1.94,方向=深度单调有益末端最优未饱和;depth=2 臂与轮 175 逐位一致=交叉验证锚);**轮 187 DEPTH-WIDTH 叠支矩阵模式**(--d_models 参数化):dm48/96×depth2/4 四单元——depth 主效应 ln=0.385/width 主效应 ln=0.530 均可分辨,交互 ln=0.011≪0.20=**两轴独立可加**;近似同参对照 depth4/dm48(29334)→2.44 vs depth2/dm96(20758)→2.11=深度效率迹象;1-seed 筛查口径 | `probe_run T1 10 -- .venv/bin/python -u benchmarks/depth_ladder_probe.py --depths 2,4 --d_models 48,96 [--out_name depth_width.json]`(~3min 实测);测试 `tests/test_depth_ladder_probe.py` |
