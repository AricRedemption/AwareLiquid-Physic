...
| `kspan_ladder_probe.py` | **k_train 跨度阶梯对照探针**(轮 191,KSPAN-LADDER):异频池 2000 步 prefix 三臂 k_train∈{4,8,16}(prefix 参数天然可注入=轮 181 哨兵条款适用);预注册 spread 判据(<1.05=UNRESOLVABLE k=8 默认充分/≥1.05=RESOLVED 报告最优跨度);实测 KSPAN_RESOLVED(4=1.88 最优/8=3.56/16=2.10,spread 1.90)=短跨度有益但非单调;**轮 194 KSPAN-LADDER-2 叠支首端补探** {1,2,4}:1/2 均差 ~1.9× ⇒ 真最优 k_train=4 内点确认(轮 191 首端附近精确化);k4 臂与轮 191 逐位一致=复现锚;族护栏 2/2 用尽;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/kspan_ladder_probe.py --spans 1,2,4 [--out_name kspan_ladder_v2.json]`(~2min 实测);测试 `tests/test_kspan_ladder_probe.py` |
