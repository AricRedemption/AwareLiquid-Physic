...
| `depth_ladder_probe.py` | **深度阶梯对照探针**(轮 185,DEPTH-LADDER):异频池 2000 步 prefix 三臂 depth∈{1,2,4}(hidden 固定,构造参数天然可注入=轮 181 哨兵条款适用);预注册 spread 判据(<1.05=UNRESOLVABLE depth=2 默认充分/≥1.05=RESOLVED 报告最优 depth);实测 DEPTH_RESOLVED(depth 4.73→3.56→2.44,spread 1.94,方向=深度单调有益末端最优未饱和;参数量混杂如实注记=深度×宽度交叉为后续候选;depth=2 臂与轮 175 逐位一致=交叉验证锚);1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/depth_ladder_probe.py`(~2min 实测);测试 `tests/test_depth_ladder_probe.py` |
