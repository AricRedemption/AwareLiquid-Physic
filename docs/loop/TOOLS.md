...
| `fastslow_probe.py` | **弹性摆快慢双时标探针**(轮 137,FASTSLOW-PROBE):probe-local 弹性摆族(dim=2 可分 H,ω_s=10/ω_p≈1 分离 10×,VV 解析真值)上裸头 10000 步训练;三轴=快模态短视距(T=2)+慢交换长视距(T=40)+粗 dt 刚性签名(文档轴);信号相对量四值机械判读;实测 A 3.2%(超 1% 门)B 122%(慢交换丢失)⇒ BOTH_FAILED,N1 时标条款量化边界(单时标适用/双时标短视距部分可用);1-seed 筛查口径 | `probe_run T1 5 -- .venv/bin/python benchmarks/fastslow_probe.py`(~2min 实测);测试 `tests/test_fastslow_probe.py` |
| `fastslow_probe_v2.py` | **双时标失败容量/训练量归因探针**(轮 143,FASTSLOW-2):两臂 hidden128+40k 步 vs hidden64+40k 步(4× 预算,容量轴分离),v1 族/真值/训练/评估原样复用共享真值池;归因表 STRUCTURAL/CAPACITY_NEEDED/BUDGET_DOMINANT 预注册钉死在代码;实测两臂一致 FAST_ONLY(快轴 3.2%→0.26%/0.20% 过门,慢轴 122%→22%/44% 未过门)⇒ BUDGET_DOMINANT=步数为因,轮 137 BOTH_FAILED 属优化限制非结构限制,N1 路由 SCOPE_LIMITED_KEPT;1-seed 筛查口径 | `probe_run T1 15 -- .venv/bin/python benchmarks/fastslow_probe_v2.py`(~2min 实测);测试 `tests/test_fastslow_v2.py` |
