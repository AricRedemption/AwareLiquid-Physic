...
| `tosa_ladder_probe.py` | **t_obs 观测窗阶梯对照探针**(轮 223,TOSA-LADDER):异频池 2000 步 prefix 四臂 t_obs∈{8,16,24,48}(prefix 参数天然可注入=轮 181 哨兵条款适用);预注册 spread 判据(<1.05=UNRESOLVABLE t_obs=24 默认充分/≥1.05=RESOLVED 报告最优窗与曲线形状);实测 TOSA_RESOLVED(8=3.40 最优/16=6.39/24=3.56/48=5.44,spread 1.88,非单调锯齿短窗有益)=与 D6 辨识下界对表(t_obs=8 已过下界且推理负担最小);1-seed 筛查口径 | `probe_run T1 10 -- .venv/bin/python -u benchmarks/tosa_ladder_probe.py`(~4min 实测);测试 `tests/test_tosa_ladder_probe.py` |
