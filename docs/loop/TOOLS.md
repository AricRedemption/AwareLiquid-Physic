...
| `dt_curriculum_probe.py` | **dt 分辨率课程对照探针**(轮 165,DT-CURRICULUM):单频池 ω=2,2000 步受限预算两臂——A 恒定 dt=0.05 vs B 课程(dt=0.1×1000→dt=0.05×1000,权重搬运 dt 重挂载,Adam 状态不继承已注记);预注册三分支判据(\|diff\|<5%⇒UNRESOLVABLE 与 Wu"随机序强基线"相容/B 好≥5%⇒BENEFICIAL/B 差⇒HARMFUL);实测 CURRICULUM_BENEFICIAL(A 0.4225 vs B 0.2796,+33.8%)且优于轮 162 矩阵两纯臂=组合收益;机制归因(排序 vs 物理覆盖)未分离=后续池候选;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/dt_curriculum_probe.py`(~2min 实测);测试 `tests/test_dt_curriculum_probe.py` |
