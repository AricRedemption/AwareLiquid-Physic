...
| `ctx_dim_ladder_probe.py` | **ctx 容量阶梯对照探针**(轮 226,CTX-DIM-LADDER):异频池 2000 步 prefix 四臂 context_dim∈{1,2,4,8}(构造参数天然可注入=轮 181 哨兵条款适用);预注册 spread 判据(<1.05=UNRESOLVABLE 默认充分/≥1.05=RESOLVED 报告最优容量);实测 CTXDIM_RESOLVED(1=1.97 最优/8=3.56,单调有害差 80%)=**默认 ctx_dim=8 是被检出的最大配置误差**,ω=1 维真值容量冗余干扰实证(§53.1);与 E4a 梯度饥饿对照;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/ctx_dim_ladder_probe.py`(~3min 实测);测试 `tests/test_ctx_dim_ladder_probe.py` |
