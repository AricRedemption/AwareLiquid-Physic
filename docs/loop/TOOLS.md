...
| `pool_width_probe.py` | **池分布宽度对照探针**(轮 175,POOL-WIDTH):M1 弹簧 2000 步 prefix 两臂——A 窄池 ω∈[0.95,1.05](近单频)vs B 宽池 ω∈[0.7,1.8](E1 默认),各评估同分布 held-out k100;预注册三分支判据(ratio∈[0.95,1.05]=UNRESOLVABLE 与 Kumar 反直觉相容/>1.05=COST/<0.95=BENEFIT);实测 WIDTH_COST(ratio=37.5):**口径拆解=任务难度+ctx 推断代价合计非纯训练多样性损害,跨池数字不可直接比(池宽度列入基线表强制列,轮 111 条款实证)**;与轮 120 单频控制臂方向一致互补;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/pool_width_probe.py`(~1min 实测);测试 `tests/test_pool_width_probe.py` |
