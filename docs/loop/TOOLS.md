...
| `nbody_pool_audit.py` | **NBody 聚合语义审计**(轮 118,NBODY-POOL-AUDIT,dir/nbody-pool-audit):闭合轮 95 声明边界——免训练 DPI 判决性设计,质量扰动中心差分 ⇒ 池化质心型 summary vs 全观测的 Fisher 比;实测 retention 1.083e-2(挤压非湮灭:≫M2 的 8.6e-11 但只留 ~1%),mean-pool 病态定性跨三线闭合;预注册判负①-③钉在代码内 | `probe_run T1 2 -- .venv/bin/python benchmarks/nbody_pool_audit.py`(秒级);测试 `tests/test_nbody_pool_audit.py`(雅可比 toy 用 float64——轮 110 条款) |
