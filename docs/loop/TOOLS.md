...
| `len_extrap_probe.py` | **训练窗外 rollout 外推探针**(轮 170,LEN-EXTRAP):异频池 160 步(16s 窗)训练 2000 步,新池 601 步(60s)held-out 单条 600 步 rollout 建 per-step 误差剖面;预注册分段比值判据(comp=外推[20,30]s/内插[5,10]s<3=ROBUST;≥3 且 16s 边界跳变≥3×=WINDOW_EDGE;否则 GRADUAL)+能量漂移诊断;实测 LEN_ROBUST(comp=0.88,边界跳变 1.01)=窗外 1.25-1.875× 平缓延续无边界效应,E-drift 29.3@60s=位置精度与能量保持解耦;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/len_extrap_probe.py`(~2min 实测);测试 `tests/test_len_extrap_probe.py` |
