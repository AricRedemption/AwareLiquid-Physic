...
| `kspan_ladder_probe.py` | **k_train 跨度阶梯对照探针**(轮 191,KSPAN-LADDER):异频池 2000 步 prefix 三臂 k_train∈{4,8,16}(prefix 参数天然可注入=轮 181 哨兵条款适用);预注册 spread 判据(<1.05=UNRESOLVABLE k=8 默认充分/≥1.05=RESOLVED 报告最优跨度);实测 KSPAN_RESOLVED(4=1.88 最优/8=3.56/16=2.10,spread 1.90)=短跨度有益但非单调(k<4 未探),与 §46.1 部署接近预言相反(机制候选=长跨度后段误差主导梯度,多 seed=停车场);k8 臂与历史逐位一致=交叉验证锚;1-seed 筛查口径 | `probe_run T1 8 -- .venv/bin/python -u benchmarks/kspan_ladder_probe.py`(~3min 实测);测试 `tests/test_kspan_ladder_probe.py` |
