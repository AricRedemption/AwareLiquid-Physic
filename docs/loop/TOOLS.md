...
| `lrbatch_grid_probe.py` | **batch×lr 缩放规则检验**(轮 205,LRBATCH-GRID):异频池 2000 步 prefix 五单元 (batch,lr) 网格;预注册等价对判据(线性对 (16,3e-3)vs(64,1.2e-2) 与平方根对 (16,3e-3)vs(64,6e-3) 各 <5%=规则成立);实测 SCALING_BROKEN(线性差 33.1%/平方根差 20.6%)=Smith 等价本仓打破,batch×lr 需联合调优不可分离外推;最好单元 (64,1.2e-2)=1.597 最差 (16,1.2e-2)=5.454(lr 效应依赖 batch=真交互);1-seed 筛查口径 | `probe_run T1 10 -- .venv/bin/python -u benchmarks/lrbatch_grid_probe.py`(~3min 实测);测试 `tests/test_lrbatch_grid_probe.py` |
