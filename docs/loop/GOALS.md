...
current_action: >-
  ...
  唯一源=AMENDMENTS+goal_check 输出(每心跳必见)。轮 280 消化轮
  完成:AMM-033 either-or 采纳注记(默认替换选候选头单独,回灌
  配方与头不叠加,两 PR 互不阻塞互不预设;排序证据链=轮 252+269+
  279)+分流干净(EXP 0.2/WIP 0/欠账 0/消化率 1.0)。下一心跳:
