...
4b-2. **双时标边界(轮 137 FASTSLOW-PROBE,[B],1-seed T1 筛查;
   PR#9 待合并)**:弹性摆族(ω_s/ω_p 分离 10×,dim=2 可分头直接表示)
   解析 dt 训练后——快模态短视距 rel 3.2%(部分捕捉,超 1% 门)、慢交换
   T=40 rel 122%(完全丢失,真值能量漂移 15.5%)⇒ N1 时标条款:
   "结构守恒+单时标家族适用;双时标系统短视距部分可用、慢交换长视距
   不可用"(§33.2 刚性签名:失效=相位累积非数值爆権);多 seed 终局=
   停车场。
