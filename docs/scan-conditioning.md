...
- **轮 122 状态行(R1D-MODE-SCAN 判读 NEGATIVE,R1d 撤回)**:先决
  到位后(R1c 轮 96 判负+SB-NAMING 对账成立 ⇒ 轮 121 入队),预言
  检验按轮 78 预注册判负分支执行——e1_final 同协议逐模式提取扫描
  实测全模式无可解码信号(corr 噪声级,e≈1;与 e1_final 0.014/D-3
  0.058/P3 链一致),序数-质量=平坦 ⇒ **R1d 撤回,本第四路删除**
  (判读全录见 PRD §19 轮 122 与 spectral-bias-naming.md §3.1;
  本轮 77 追加文本按"新增不改史"保留作历史)。分流表现存三路
  (R1 监督/R1b 结构/R1c 聚合),R1b 维持条件性登记(轮 95 裁决)。
