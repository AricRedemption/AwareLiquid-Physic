...
## 60. 经验蒸馏 56(轮 273,2026-09-26,QUEUE-EMPTY 蒸馏轮):长视距守恒/稳定性族([行动] 轮=HOM-DRIFT 入队执行)

> 钩子=AMM-033 锚重置计划的长视距维度缺口:替换候选的读数止于
> k100 分布内/k200 外推(轮 268/269,dir 分支 PR#49/#50;同段
> §59 表达力边界族见 dir/hom-bound PR#51),结构保持文献预测
> 约束模型长期能量误差有界 vs 通用模型长期漂移——替换默认头
> 前必须测长视距。族=学习动力学模型的长视距守恒与稳定性。
> 机制核对:Symplectic RNN(arXiv:1909.13334)已在库不重复收;
> §22.2 Symplectic Adjoint=梯度方法面分立;LEN-EXTRAP 族=长度
> 外推稳健性(默认配置,无守恒读数)分立;house P0-3 能量漂移
> =协议面非文献族。

### 60.1 结构保持⇒长期能量误差有界 [坐标] ★强

- 【出处】Jin, Zhang, Zhu, Tang & Karniadakis, "SympNets: Intrinsic
  structure-preserving symplectic networks for identifying
  Hamiltonian systems", Neural Networks 132:166-179, 2020(题录
  当场核验:期刊卷期+作者;常被误标 NeurIPS 2020,以期刊为准)
- 【内容】内嵌辛结构(leapfrog 模块组合)的网络长期预测能量
  误差有界(阴影哈密顿量机制),与通用网络积分器的长期漂移
  形成对照。
- 【对我们的映射】齐次 V 约束头 vs 默认头的长视距守恒对照=
  本族行动面;AMM-033 锚重置计划的长视距维度。
- 【适用条件】AMM-033 锚集设计依据。
- 【验证状态】题录当场核验;社区已验证(高引基线)。

### 60.2 稳定性认证学习 [坐标]

- 【出处】Manek & Kolter, "Learning Stable Deep Dynamics Models",
  NeurIPS 2019(arXiv:2001.06116,~370 引;题录当场核验,常被
  误标 2020,以会议年为准)
- 【内容】经 Lyapunov 函数结构注入学习稳定动力学(原点稳定
  保证)。
- 【对我们的映射】"结构先验⇒长期有界性"的另一谱系(稳定性
  侧,非辛侧);判读分支的文献方向校准。
- 【适用条件】同 60.1。
- 【验证状态】题录当场核验;社区已验证。
...
- 【出处】Cranmer, Greydanus, Hoyer, Battaglia, Spergel & Ho,
  "Lagrangian Neural Networks", arXiv:2003.04630, 2020(~956 引;
  题录当场核验;ICLR 2020 DeepDiffEq workshop,venue 弱如实
  注记)
- 【内容】任意拉格朗日量的神经网络参数化(非正则坐标泛化)。
- 【对我们的映射】保守量学习谱系参照;非本探针行动面,
  长视距诊断透镜。
- 【适用条件】同 60.1。
- 【验证状态】题录当场核验(arXiv);workshop venue。

### 蒸馏结论

- 【坐标】×3 入库(§60.1-60.3)。
- 【判定】有 [行动]:HOM-DRIFT 入队执行(E1 池双臂 A=默认头
  vs B=候选头 ×3-seed ×视距 k{100,400,1000},MSE+能量漂移双
  读数;决策耦合=AMM-033 锚重置计划长视距维度三路修订);
  S1 重置([行动])。蒸馏第 56 次达标。
