...
## 62. 经验蒸馏 58(轮 277,2026-09-26,QUEUE-EMPTY 蒸馏轮):归纳偏置×样本效率族([行动] 轮=HOM-SAMPLE 入队执行)

> 钩子=齐次头证据链的未测维度:约束头的经典价值命题=小样本端
> 优势更强(归纳偏置在数据稀缺时最有价值)——候选头在 house 规模
> (256)已赢(轮 269 ratio 0.66),规模轴从未测。族=归纳偏置×
> 样本效率。机制核对:P1 样本效率=house 协议面非文献族;§45
> 深度容量/§61 盆地稳定性分立;同族第 60 族=长视距轴分立。

### 62.1 数据受限体制的归纳偏置 [坐标] ★强

- 【出处】Mialon, "On Inductive Biases for Machine Learning in
  Data-Constrained Settings", arXiv:2302.10692(2023,题录当场
  核验:arXiv id+HAL 存档)
- 【内容】归纳偏置=对数据空间的假设约束;对称性等约束换取
  数据受限体制的样本复杂度改善(综述论文)。
- 【对我们的映射】候选头(次数匹配约束)vs 默认头的数据规模
  阶梯=该命题的玩具标度直接测量。
- 【适用条件】AMM-033 benefit-scope 注记依据。
- 【验证状态】题录当场核验(论文综述,社区引用广泛)。

### 62.2 等变的可证严格泛化收益 [坐标]

- 【出处】Elesedy & Zaidi, "Provably Strict Generalisation Benefit
  for Equivariant Models", ICML 2021(题录当场核验;**arXiv id
  勘误=2102.10333**,先前记忆 2102.07945 有误如实注记)
- 【内容】等变模型(群平均特征)对非等变最小二乘基线的第一
  个可证严格泛化收益(样本复杂度)。
- 【对我们的映射】"约束⇒样本效率"的理论正典;判读分支的
  文献方向校准。
- 【适用条件】同 62.1。
- 【验证状态】题录当场核验;社区已验证。
...
- 【出处】Lyle, van der Wilk, Kwiatkowska, Gal & Bloem-Reddy, "On
  the Benefits of Invariance in Neural Networks", arXiv:2005.00178
  (2020,~131 引;题录当场核验)
- 【内容】不变性收益的一般框架:不变模型/数据增强/特征平均
  三路的样本效率量化比较。
- 【对我们的映射】约束收益的度量框架参照;HOM-SAMPLE 的
  规模阶梯即其样本效率轴的最小实现。
- 【适用条件】同 62.1。
- 【验证状态】题录当场核验(arXiv);社区已验证。

### 蒸馏结论

- 【坐标】×3 入库(§62.1-62.3)。
- 【判定】有 [行动]:HOM-SAMPLE 入队执行(2 头×3 数据规模×
  3-seed;决策耦合=AMM-033 benefit-scope/锚注记三路修订);
  S1 重置([行动])。蒸馏第 58 次达标。
