...
## 61. 经验蒸馏 57(轮 275,2026-09-26,QUEUE-EMPTY 蒸馏轮):训练盆地稳定性/损失景观族([行动] 轮=POOL-BITS 入队执行)

> 钩子=轮 273 意外实证的纹理:池张量长度(160→1100,同 seed 同
> draws 同系统,仅超越函数低位比特异)让默认头 MSE 逐 seed 摆动
> 最高 +71%,候选头仅 ~0.05%-16%——盆地稳定性从未系统测量,直接
> 关系 AMM-033 锚集变体协议(锚多少变体才可信)。族=训练盆地
> 稳定性/损失景观。机制核对:§58 种子方差感知采纳族=采纳协议面
> 分立(61=盆地稳定性机制面);轮 126 house seed 敏感性=实验
> 记录非文献族。

### 61.1 线性模式连通性/排列不变性 [坐标] ★强

- 【出处】Entezari, Sedghi, Saukh & Neyshabur, "The Role of
  Permutation Invariance in Linear Mode Connectivity of Neural
  Networks", ICLR 2022(题录当场核验:venue+作者)
- 【内容】独立训练的网络经神经元排列对齐后几乎总是线性模式
  连通——盆地结构对训练随机性的鲁棒性刻画。
- 【对我们的映射】池比特扰动=最小训练随机性扰动;头间稳定
  性差异=盆地几何的玩具标度读数。
- 【适用条件】AMM-033 锚集变体协议依据。
- 【验证状态】题录当场核验;社区已验证。

### 61.2 彩票假设/初始化盆地 [坐标]

- 【出处】Frankle & Carbin, "The Lottery Ticket Hypothesis: Finding
  Sparse, Trainable Neural Networks", ICLR 2019 最佳论文(题录
  当场核验)
- 【内容】密集网络的训练结果强依赖初始化盆地;稀疏子网络在
  同初始化下可复现训练。
- 【对我们的映射】默认头 vs 候选头对数据微扰的敏感度差异=
  初始化/数据耦合盆地宽度的实证面。
- 【适用条件】同 61.1。
- 【验证状态】题录当场核验;社区已验证(最高引级)。

### 61.3 尖锐极小与泛化解耦 [坐标]

- 【出处】Dinh, Pascanu, Bengio & Bengio, "Sharp Minima Can
  Generalize For Deep Nets", ICML 2017, arXiv:1703.04933
  (~1170 引;题录当场核验)
- 【内容】平坦度度量在重参数化下不稳定,尖锐极小也可泛化。
- 【对我们的映射】稳定性读数≠质量读数的诚实边界:POOL-BITS
  只测盆地稳定性,不据此断言泛化优劣(与轮 269 外推读数分立)。
- 【适用条件】同 61.1。
- 【验证状态】题录当场核验;社区已验证。
...
- 【坐标】×3 入库(§61.1-61.3)。
- 【判定】有 [行动]:POOL-BITS 入队执行(4 池变体×2 头×3-seed
  的盆地稳定性测量;决策耦合=AMM-033 锚集变体协议三路修订);
  S1 重置([行动])。蒸馏第 57 次达标。
